 ...
 * @version   Release: 1.0
 * @link      http://www.php.net/manual/fr/book.pdo.php PHP Data Objects sur php.net
 * @var String $serveur propriété contenant le nom du serveur mysql à utiliser lors de la création de l'objet PDO
 * @var String $bbd propriété contenant le nom du la base de données mysql à utiliser lors de la création de l'objet PDO
 * @var String $user propriété contenant le nom de l'utilisateur mysql à utiliser lors de la création de l'objet PDO
 * @var String $mdp propriété contenant le mot de passe de l'utilisateur mysql à utiliser lors de la création de l'objet PDO
 * @var String $monPdo propriété valorisée lors de la création de l'objet PDO 
 * @var String $monPdoGsb propriété contenant l'instance unique de l'objet PDO 
 */
class PdoGsb
{
     ...
     * hors forfait concernées par les arguments.
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme aaaamm
     ...
     * d'un tableau associatif
     */
    public function getLeFraisHorsForfait($idVisiteur, $mois, $id)
    {
        $requetePrepare = PdoGsb::$monPdo->prepare(
            'SELECT * FROM lignefraishorsforfait '
            . 'WHERE lignefraishorsforfait.idvisiteur = :unIdVisiteur '
    ...
    /**
     * Retourne tous les id, noms et prénoms de la table visiteur sauf ceux des
     * comptables
     *
    ...
    public function getLesVisiteurs()
    {
        $requetePrepare = PdoGsb::$monPdo->prepare(
            ...
            . 'FROM visiteur WHERE comptable = 0 '
     ...
     /**
     * Retourne le nom, le prénom et l'id d'un visiteur de la table visiteur 
     * @param String $idVisiteur id du visiteur
     * @return un tableau associatif
     */
    public function getLeVisiteur($idVisiteur)
    {
        $requetePrepare = PdoGsb::$monPdo->prepare(
            ...
            . 'visiteur.nom AS nomVisiteur, '
            . 'visiteur.prenom AS prenomVisiteur '
            . 'FROM visiteur WHERE id=:idVisiteur '
            . 'ORDER BY visiteur.nom'
        );
     ...
     *
     * @param String $idVisiteur ID du visiteur
     * @param String $mois       Mois sous la forme jj/mm/aaaa
     * @param Array  $lesFrais   tableau contenant autant de lignes que de frais
     * hors forfait
     *
     * @return null
     */
    ...
    public function majFraisHorsForfait($idVisiteur, $mois, $lesFrais)
    {
        ...
        foreach ($lesCles as $unidFraisHorsForfait) {
            $id = $unidFraisHorsForfait;
            $dateAnglais = dateFrancaisVersAnglais($lesFrais[$unidFraisHorsForfait]['date']);
            $libelle = $lesFrais[$unidFraisHorsForfait]['libelle'];
            $montant = $lesFrais[$unidFraisHorsForfait]['montant'];
            $requetePrepare = PdoGSB::$monPdo->prepare(
            ...
            $requetePrepare->bindParam(':date', $dateAnglais, PDO::PARAM_STR);
    ...
    /**
     * Met à jour la table ligneFraisHorsForfait
     * Met à jour la table suite au refus d'une fiche de frais hors forfait
     *
     * @param Integer $id ID du frais à "supprimer"
     *
     * @return null
     */
    ...
    /**
     * Supprime le frais hors forfait dont l'id est passé en argument en vérifiant
     * qu'il s'agisse d'un frais du visiteur qui demande la suppression
     *
    ...
    /**
     * Retourne les fiches de frais d'un visiteur mises en paiement
     *
     * @param String $idVisiteur ID du visiteur
     *
     * @return un tableau
